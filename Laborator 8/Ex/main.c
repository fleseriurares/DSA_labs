...
int *sortare_topologica(Graf *G)
{
    int *a=(int *)malloc((G->n)*sizeof(int));
    for(int i=0; i<G->n; i++)
    {
        a[i]=G->f[i];
    }
...
int main()
{
    ...
    print(G.pi, G.n);
    ///printf("\n*GRAFUL CONTINE SI NODUL 0, CEEA CE POATE CREA CONFUZIE LA VECTORUL DE PARINTI.*");
    ///printf("\n*VARIANTA RECURSIVA SI ITERATIVA POT SA DIFERE IN CEEA CE AFISEAZA, DATORITA FAPTULUI CA GRAFUL ESTE NEORIENTAT.*");
